...
USE smarkio_credz;
DELIMITER |
CREATE TRIGGER tg_transbordo_retencao_atraso_negociar AFTER INSERT ON leads
FOR EACH ROW
BEGIN
	-- VERIFICA SE O REGISTRO PARA DATA E CANPAIGN E MENU EXISTE
	IF (SELECT EXISTS(SELECT * FROM smarkio_credz.transbordo_retencao_atraso_negociar_tg AS t
		WHERE t.date = NEW.lead_creation_day 
        AND t.campaign = CASE WHEN NEW.campaign IS NULL OR NEW.campaign = '' THEN '-' ELSE NEW.campaign END
        AND t.atraso_negociar = CASE WHEN NEW.atraso_negociar IS NULL OR NEW.atraso_negociar = '' THEN '-' ELSE NEW.atraso_negociar END)=0
        AND NEW.atraso_negociar IS NOT NULL) 
    THEN INSERT INTO smarkio_credz.transbordo_retencao_atraso_negociar_tg
		(date,campaign,atraso_negociar)
		VALUES 
		(NEW.lead_creation_day, 
        CASE WHEN NEW.campaign IS NULL OR NEW.campaign = '' THEN '-' ELSE NEW.campaign END,
        CASE WHEN NEW.atraso_negociar IS NULL OR NEW.atraso_negociar = '' THEN '-' ELSE NEW.atraso_negociar END
        );
    END IF;
    -- MÉTRICAS
    -- TRANSBORDO
    IF((NEW.transferencia_humano LIKE "%" OR NEW.tranferencia_humano LIKE "%")
        ...
        UPDATE smarkio_credz.transbordo_retencao_atraso_negociar_tg
        SET transbordo = transbordo + 1
        WHERE date = NEW.lead_creation_day 
        AND campaign = CASE WHEN NEW.campaign IS NULL OR NEW.campaign = '' THEN '-' ELSE NEW.campaign END
        AND atraso_negociar = CASE WHEN NEW.atraso_negociar IS NULL OR NEW.atraso_negociar = '' THEN '-' ELSE NEW.atraso_negociar END;
    END IF;
    -- INTERACAO (17 a 109)
    ...
    END IF;
    -- TOTAL
    IF (NEW.atraso_negociar IS NOT NULL) THEN
        UPDATE smarkio_credz.transbordo_retencao_atraso_negociar_tg
        SET total = total + 1
        WHERE date = NEW.lead_creation_day 
        AND campaign = CASE WHEN NEW.campaign IS NULL OR NEW.campaign = '' THEN '-' ELSE NEW.campaign END
...
USE smarkio_credz;
DELIMITER |
CREATE TRIGGER tg_transbordo_retencao_atraso_negociar_update AFTER UPDATE ON leads
FOR EACH ROW
BEGIN
    IF (SELECT EXISTS(SELECT * FROM smarkio_credz.transbordo_retencao_atraso_negociar_tg AS t
		WHERE t.date = NEW.lead_creation_day 
    ...
    IF((OLD.transferencia_humano LIKE "%" OR OLD.tranferencia_humano LIKE "%")
		...
		OR (DATE_FORMAT(OLD.created_at, "%H:%i:%s") BETWEEN "09:00:00" AND "14:00:00"
				...
				AND OLD.campaign = "Negociação"
                AND OLD.identification_number1 IS NOT NULL)
		AND OLD.atraso_negociar IS NOT NULL
		) THEN
        UPDATE smarkio_credz.transbordo_retencao_atraso_negociar_tg
        SET transbordo = transbordo - 1
        WHERE date = OLD.lead_creation_day 
        AND campaign = CASE WHEN OLD.campaign IS NULL OR OLD.campaign = '' THEN '-' ELSE OLD.campaign END
        AND atraso_negociar = CASE WHEN OLD.atraso_negociar IS NULL OR OLD.atraso_negociar = '' THEN '-' ELSE OLD.atraso_negociar END;
    END IF;
    -- INTERACAO (17 a 109)
    ...
    END IF;
    -- TOTAL
    IF (OLD.atraso_negociar IS NOT NULL) THEN
        UPDATE smarkio_credz.transbordo_retencao_atraso_negociar_tg
        SET total = total - 1
        WHERE date = OLD.lead_creation_day 
        AND campaign = CASE WHEN OLD.campaign IS NULL OR OLD.campaign = '' THEN '-' ELSE OLD.campaign END
