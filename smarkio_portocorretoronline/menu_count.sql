...
FOR EACH ROW
BEGIN
    IF ((NEW.menu_inicial IS NOT NULL) 
    AND (SELECT EXISTS (
      SELECT * FROM smarkio_portocorretoronline.menu_count 
      WHERE date = NEW.lead_creation_day 
      AND menu = CASE WHEN (NEW.menu_inicial IS NOT NULL) THEN NEW.menu_inicial ELSE '-' END)=0))
        
      THEN INSERT INTO smarkio_portocorretoronline.menu_count
      (date, menu)
      VALUES (NEW.lead_creation_day, CASE WHEN (NEW.menu_inicial IS NOT NULL) THEN NEW.menu_inicial ELSE '-' END);
    END IF;

    IF (NEW.menu_inicial IS NOT NULL) THEN 
      UPDATE smarkio_portocorretoronline.menu_count
      SET acessos = acessos + 1 
      WHERE date = NEW.lead_creation_day 
      AND menu = CASE WHEN (NEW.menu_inicial IS NOT NULL) THEN NEW.menu_inicial ELSE '-' END;
    END IF;	
...
BEGIN
    ...
    IF (OLD.menu_inicial IS NOT NULL) THEN 
      UPDATE smarkio_portocorretoronline.menu_count
      SET acessos = acessos - 1 
      WHERE date = OLD.lead_creation_day 
      AND menu = CASE WHEN (OLD.menu_inicial IS NOT NULL) THEN OLD.menu_inicial ELSE '-' END;
    END IF;	
