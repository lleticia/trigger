...
-- TABLE -- 
  CREATE TABLE `smarkio_portoseguroriscosfinanceiros`.`assuntos_count` (
...
BEGIN
    IF ((NEW.susep != 'COL10J') 
        AND ((NEW.menu_principal_cap IS NOT NULL) OR (NEW.menu_principal_fianca IS NOT NULL) OR (NEW.menu_principal_garantia IS NOT NULL))
        AND (SELECT EXISTS (
          SELECT * FROM smarkio_portoseguroriscosfinanceiros.assuntos_count 
    ...
    THEN INSERT INTO smarkio_portoseguroriscosfinanceiros.assuntos_count
    (date, assunto, produto)
    ...
    IF (NEW.susep != 'COL10J') THEN 
        UPDATE smarkio_portoseguroriscosfinanceiros.assuntos_count
...
USE smarkio_portoseguroriscosfinanceiros;
DELIMITER |
CREATE TRIGGER tg_assuntos_up AFTER UPDATE ON leads
FOR EACH ROW
BEGIN
	  IF ((NEW.susep != 'COL10J') 
        AND ((NEW.menu_principal_cap IS NOT NULL) OR (NEW.menu_principal_fianca IS NOT NULL) OR (NEW.menu_principal_garantia IS NOT NULL))
...
-- SELECT -- 
INSERT INTO smarkio_portoseguroriscosfinanceiros.assuntos_count (`date`, `assunto`, `produto`, `transbordo`, `interacao`)
SELECT c.date, c.assunto, c.produto, c.transbordo, c.interacao
FROM 
(
SELECT 
    ...
    AND susep != 'COL10J'
    AND ((menu_principal_cap IS NOT NULL) OR (menu_principal_fianca IS NOT NULL) OR (menu_principal_garantia IS NOT NULL))
	GROUP BY date, produto, assunto) AS c 
